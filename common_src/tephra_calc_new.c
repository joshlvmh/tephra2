...
{
  int64_t tmp = (long)(1512775 * x + 1072632447);
  int64_t index = (int64_t)(tmp >> 12) & 0xFF;
  tmp = tmp << 32;
  double res = *(double*)&tmp;
  return res * ExpAdjustment[index];
}
...
double get_rho(double part_size, GRAIN *gr)

{
	...
	#ifdef _PRINT
  fprintf(log_file, "IN get_rho ...\n");
   fflush(log_file);  
#endif

	size1 = gr[i-1].phi_size;
	size2 = gr[i].phi_size;
	cum1  = gr[i-1].content;
	cum2  = gr[i].content;
	...
	/*printf("here I come; func_rho= %1.4f\n", func_rho);*/
    	...
    	#ifdef _PRINT
  fprintf(log_file, "EXIT  get_rho ... %g\n", func_rho);
   fflush(log_file);  
#endif
    return func_rho;
}
...
double pdf_grainsize(double part_mean_size, double part_size_slice, double part_step_width, GRAIN *gr) {
  
  double func_rho, temp;
  double demon3, demon2;
 
#ifdef _PRINT
 fprintf(log_file, "IN pdf_grainsize\n");
 #endif
	if(GRAIN_DATA == 1) { /* added by Laura (Oct, 2014) */
		func_rho = get_rho(part_size_slice, gr) * part_step_width;
		#ifdef _PRINT
		 fprintf(log_file, "RETURN=%g\n", func_rho);
 		#endif
	} else {
    ...
    fprintf(log_file,
    "[method pdf_grainsize] error: func_rho=%g]: demon2=%g step_width=%g temp=%g\n", 	func_rho, demon2, part_step_width, temp); 
    #endif
	}
...
double strat_average(
    ...
    demon3 = FastExp(-demon1/sigma); /* where sigma is calculated for the total fall time */
    return demon3;
}
...
#ifdef _PRINT  
fprintf(log_file, "IN [set_global_values] ... .... OUT\n");
#endif
}

void set_eruption_values(ERUPTION *erupt, WIND *wind, GRAIN *gr) { /* set_eruption_values */
...
#ifdef _PRINT
fprintf(log_file, "IN set_eruption_values ... %d\n", PART_STEPS);
 fflush(log_file);  
#endif
...
/*    x = erupt->vent_elevation;
    ...
    */
    ...
    for (i=0; i < PART_STEPS; i++) {
        ...
        cum_prob_part += prob;
         #ifdef _PRINT
    fprintf(log_file, "Cum_Prob=[%d]%g\t%g\n", i, cum_prob_part, prob);
     fflush(log_file);  
     #endif
        y += part_step_width;
    }
...
/************************End of normalization steps  ******************************************/
    ...
    */
    ...
    for (i = 0; i < PART_STEPS; i++) { /* PART_STEPS_LOOP */
        ...
        part_prob = pdf_grainsize(erupt->mean_phi, y, part_step_width, gr);
        cum_fall_time = 0.0;
        wind_x = 0.0;
        wind_y = 0.0;
