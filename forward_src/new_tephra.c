...
static WIND **W;
static POINT *pt;
static GRAIN *g_ptr;	/* added by Laura (Oct, 2014) */
...
static int num_grain = 0; /*total number of grain sizes used in the analysis */
...
FILE *in_wind;
FILE *log_file;
FILE *in_grain; /* added by Laura (Oct, 2014) */
...
int main(int argc, char *argv[]) { /* MAIN CODE STARTS HERE */
  ...
  fprintf(stderr, "Opened log file:%s for writing ....\n", log_name);
  ...
  if (log_file == NULL) {
	...
	fprintf(log_file, "Opened %s for writing ....\n", log_name);
  ...
  */
  ...
  /* READ GRAINSIZE FILE - added by Laura (Oct, 2014) */
	if (argc == 5)
	{
			fprintf(stderr, "Using custom grain data: %s\n", argv[4]);
	...
	{
		fprintf(stderr, "Using Gaussian distribution for tephra dispersion\n"); 
	}
  ...
  set_eruption_values(&e, W[0], g_ptr);
#ifdef _PRINT
  fflush(log_file); 
#endif
  for (i = 0; i < num_pts; i++)   /* For each location */
     tephra_calc(&e, pt+i, W[0], &stats, g_ptr);
 #ifdef _PRINT  
...

{
	...
	char line[MAX_LINE];
	int max_gs=0, min_gs=0; 

	#ifdef _PRINT
  fprintf(log_file,"ENTER[get_grain]\n");
#endif
...
/* count total number of grain size data */	
	while (NULL != fgets(line, MAX_LINE, in))
	{
		...
		#ifdef _PRINT
		fprintf(log_file, "[%d] line = %s", num_grain, line);
		#endif
		/*++total_line_number;*/
		num_grain++;
	}
	...
	g_ptr = (GRAIN *)GC_MALLOC((size_t)(num_grain+1) * sizeof(GRAIN));
	if (g_ptr == NULL)
	{
		fprintf(log_file, 
				"Cannot malloc memory for Grain Size Table:[%s]\n",
				strerror(errno));
		exit(1);
    }
	...
	while (NULL != fgets(line, MAX_LINE, in))
	{
		...
		g_ptr[i].phi_size = grain_size;
		g_ptr[i].content   = grain_cont;
		
		/* Seb */
		// fprintf(stderr, "Grainsize = %f\n", grain_size);
		// fprintf(stderr, "Graincont = %f\n", grain_cont);
		// fprintf(stderr, "Grainsize = %d\n", total_line_number);
		 fprintf(log_file, "[%d] %f %f\n", 
		 			i, g_ptr[i].phi_size, g_ptr[i].content);
		/* Function added by seb to get MAX and MIN grainsize of a custom grainsize distribution */
		if (i == 0)
		{
			max_gs = g_ptr[i].phi_size;
			fprintf(stderr, "MAX_GRAINSIZE = %d\n", max_gs);
		}	
		/*else if (i == total_line_number-1)*/
		else if (i == num_grain-1)
		{
			min_gs = g_ptr[i].phi_size;
			fprintf(stderr, "MIN_GRAINSIZE = %d\n", min_gs);
		}

		/*printf("line 115, line = %s sizeclass = %1.1f sizecontent = %1.1f\n", line, g_ptr[i].sizeclass, g_ptr[i].content);
		*/
		++i;
	}
	/*fprintf(stderr, "total line number = %d\n", total_line_number);*/
	
	/* Added by Seb, updates global values */
	e.min_phi = max_gs;
	e.max_phi = min_gs;
	
#ifdef _PRINT
  fprintf(log_file,"EXIT[get_grain]\n");
#endif
	return 0;
